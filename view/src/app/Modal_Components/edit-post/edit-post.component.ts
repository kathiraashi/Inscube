...
export class EditPostComponent implements OnInit {
  ...
  constructor(
    ...
    this.Cube_Service.User_Followed_Cubes(this.LoginUser._id).subscribe(
      datas => {
  ...
  ngOnInit(): void {
    ...
    if (
      this.data.Post_Info.Attach_File !== undefined &&
      this.data.Post_Info.Attach_File.length > 0
    ) {
      this.Old_File = this.data.Post_Info.Attach_File;
    ...
    this.Form = this.formBuilder.group({
      ...
      Post_Type: new FormControl(
        this.data.Post_Info.Post_Category,
        Validators.required
      ),
  ...
  Select_Cube(index) {
    if (this.Cubes_List[index].Selected) {
      ...
      const _index = this.Selected_Cube.findIndex(
        x => x === this.Cubes_List[index]._id
      );
  ...
  onSubmit() {
    if (this.Form.valid && this.Selected_Cube.length > 0) {
      ...
      if (this.Old_File.length > 0) {
        this.FormData.set('Old_Attach_File', JSON.stringify(this.Old_File[0]));
      }

      if (this.Attachment_File.length > 0 && this.FileInputActive) {
        this.FormData.append(
          'attachments',
          this.Attachment_File[0],
          this.Attachment_File[0].name
        );
      }
      ...
      if (
        this.Old_Img_Preview.length > 0 ||
        this.Old_Video_Preview.length > 0
      ) {
        let Old_Files = [];
